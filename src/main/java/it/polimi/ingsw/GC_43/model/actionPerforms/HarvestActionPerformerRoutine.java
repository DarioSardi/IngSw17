...
public class HarvestActionPerformerRoutine implements ActionPerformer {
	...
	public boolean performAction() {
		...
		if(checkResult==true){
			...
			System.out.println("Resetting player resources to inital ones and setting family member used free again..");
...
private void checkHarvestPerform(Player player, FamilyMember familyMember) {
	...
	for(TerritoryCard territoryCard: this.harvestAction.getPlayer().getPlayerCards().getArrayTerritoryCards()){
		if(dieValue>=territoryCard.getProductionDice()){
			...
			System.out.println("Executing permanent bonus of "+territoryCard.getCardName()+" card which min die value is "+territoryCard.getProductionDice()+ " & familiar die value is "+dieValue);
