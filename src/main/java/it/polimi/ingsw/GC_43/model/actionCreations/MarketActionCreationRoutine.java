...
public class MarketActionCreationRoutine implements ActionCreation {
	...
	*/
	...
	public boolean prepareAction() {
		this.marketAction
				.setFamilyMember(CommonActionCreatorRoutine.askForFamilyMemberChoice(this.marketAction.getPlayer()));
		this.marketAction.setFamilyMemberColor(this.marketAction.getFamilyMember().getColor());
		this.marketAction.setServantsUsed(CommonActionCreatorRoutine.askForServantsUsage(marketAction.getPlayer(),
				this.marketAction.getFamilyMember().getDiceValue()));
	...
	private void getMarketChoices(FamilyMember familyMember) {
		...
		if (this.checkResult) {
			...
			System.out.println("Unfortunately all market spaces are occupied, action will be cancelled..");
		}
	}

	private void checkIfAllSpacesAreOccupied() {
		int occupiedSpaces = 0;

		for (MarketActionSpace a : this.board.getMarket().getMarketActionSpaces()) {
			if (a.isOccupied()) {
				occupiedSpaces++;
			}
		}
		if (occupiedSpaces == this.board.getMarket().getMarketActionSpaces().size()) {
